        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 dark:bg-slate-950/40 backdrop-blur-md animate-fade-in"
            ...
            <div @click.outside="document.getElementById('modal-close-btn').click()"
                 ...
                 <div class="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-transparent">
                     <div>
                         <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">Catat Transaksi</h2>
                         <p class="text-[13px] font-medium text-slate-500 dark:text-slate-400 mt-1">Rekam pemasukan atau pengeluaran organisasi</p>
                     </div>
                 ...
                 <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                     <form action="{{ route('keuangan.store') }}" method="POST" data-turbo-frame="_top" class="space-y-6">
                         @csrf
                         ...
                         <div>
                             ...
                             <input type="text" id="judul" name="judul" value="{{ old('judul') }}"
                                    ...
                                    placeholder="Contoh: Iuran Bulanan Maret" required autofocus>
                         ...
                         <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                             ...
                             <div>
                                 <label for="jenis" class="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-2 pl-1">Jenis Transaksi <span class="text-rose-500">*</span></label>
                                 ...
                                 @error('jenis')<p class="mt-2 text-[12px] font-bold text-rose-500 pl-1">{{ $message }}</p>@enderror
                             </div>
                         </div>
                             ...
                             <div class="relative">
                                 <select id="kegiatan_id" name="kegiatan_id" class="w-full h-12 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-transparent focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:focus:border-blue-500/50 rounded-xl px-4 text-[14px] font-medium text-slate-900 dark:text-white transition-all appearance-none">
                                     ...
                                     @foreach($kegiatan as $k)
                                         <option value="{{ $k->id }}" {{ old('kegiatan_id') == $k->id ? 'selected' : '' }}>{{ $k->judul }}</option>
                                     @endforeach
                                 </select>
                         ...
                         <div class="pt-6 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-white/5">
                             ...
                             <button type="submit" class="px-6 h-11 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[13px] rounded-xl shadow-lg shadow-blue-500/30 dark:shadow-blue-500/20 active:scale-95 transition-all">Simpan Transaksi</button>
                         </div>
                     </form>
                 </div>
