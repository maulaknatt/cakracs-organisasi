        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 dark:bg-slate-950/40 backdrop-blur-md animate-fade-in"
            ...
            <div @click.outside="document.getElementById('modal-close-btn').click()"
                 ...
                 <div class="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-transparent">
                     <div>
                         <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">Edit Kegiatan</h2>
                         <p class="text-[13px] font-medium text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi kegiatan</p>
                     </div>
                     <a href="{{ route('kegiatan.show', $kegiatan->id) }}" data-turbo-frame="_top" id="modal-close-btn"
                 ...
                 <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                     <form action="{{ route('kegiatan.update', $kegiatan->id) }}" method="POST" data-turbo-frame="_top" class="space-y-6">
                         @csrf
                         @method('PUT')
                         ...
                         <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                             <div>
                                 ...
                                 <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', $kegiatan->tanggal_mulai) }}"
                             ...
                             <div>
                                 ...
                                 <input type="date" id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai', $kegiatan->tanggal_selesai) }}"
                         ...
                         <div class="pt-6 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-white/5">
                             <a href="{{ route('kegiatan.show', $kegiatan->id) }}" data-turbo-frame="_top" class="flex items-center justify-center px-6 h-11 rounded-xl text-[13px] font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 dark:hover:text-white transition-all">Batal</a>
