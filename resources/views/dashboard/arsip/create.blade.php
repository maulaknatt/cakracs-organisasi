<x-admin-layout title="Buat Arsip">
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 dark:bg-slate-950/40 backdrop-blur-md animate-fade-in"
            ...
            <div @click.outside="document.getElementById('modal-close-btn').click()"
                 ...
                 <div class="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-transparent">
                     <div>
                         <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">Buat Folder Arsip</h2>
                         <p class="text-[13px] font-medium text-slate-500 dark:text-slate-400 mt-1">Buat folder baru untuk menampung dokumen</p>
                     </div>
                     <a href="{{ route('arsip.index') }}" data-turbo-frame="_top" id="modal-close-btn"
                 ...
                 <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                     <form action="{{ route('arsip.store') }}" method="POST" data-turbo-frame="_top" class="space-y-6">
                         @csrf

                         <div>
                             <label for="judul" class="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-2 pl-1">Nama Folder <span class="text-rose-500">*</span></label>
                             <input type="text" id="judul" name="judul" value="{{ old('judul') }}"
                                    ...
                                    placeholder="Contoh: LPJ Proker A 2025" required autofocus>
                         ...
                         <div>
                             ...
                             <textarea id="deskripsi" name="deskripsi" rows="3"
                                       ...
                                       placeholder="Jelaskan isi folder arsip ini...">{{ old('deskripsi') }}</textarea>
                             @error('deskripsi')<p class="mt-2 text-[12px] font-bold text-rose-500 pl-1">{{ $message }}</p>@enderror
                         </div>

                         <div>
                             <label for="kegiatan_id" class="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-2 pl-1">Tautkan ke Kegiatan <span class="text-slate-400 dark:text-slate-500 text-[11px] font-normal ml-1">(opsional)</span></label>
                             <div class="relative">
                                 <select id="kegiatan_id" name="kegiatan_id" class="w-full h-12 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-transparent focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:focus:border-blue-500/50 rounded-xl px-4 text-[14px] font-medium text-slate-900 dark:text-white transition-all appearance-none cursor-pointer">
                                     <option value="" class="dark:bg-slate-900 dark:text-white">Umum — Tidak terkait kegiatan spesifik</option>
                                     @foreach($kegiatan as $k)
                                         <option value="{{ $k->id }}" class="dark:bg-slate-900 dark:text-white" {{ old('kegiatan_id') == $k->id ? 'selected' : '' }}>{{ $k->judul }}</option>
                                     @endforeach
                                 </select>
                         ...
                         <div class="pt-6 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-white/5">
                             <a href="{{ route('arsip.index') }}" data-turbo-frame="_top" class="flex items-center justify-center px-6 h-11 rounded-xl text-[13px] font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 dark:hover:text-white transition-all">Batal</a>
                             <button type="submit" class="px-6 h-11 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[13px] rounded-xl shadow-lg shadow-blue-500/30 dark:shadow-blue-500/20 active:scale-95 transition-all">Buat Folder</button>
                         </div>
                     </form>
                 </div>
