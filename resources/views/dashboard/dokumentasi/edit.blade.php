<x-admin-layout title="Edit Dokumentasi">
        ...
        <div class="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 bg-slate-900/40 dark:bg-slate-950/40 backdrop-blur-md animate-fade-in"
            ...
            <div @click.outside="document.getElementById('modal-close-btn').click()"
                 ...
                 <div class="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-transparent">
                     <div>
                         <h2 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">Edit Dokumentasi</h2>
                         <p class="text-[13px] font-medium text-slate-500 dark:text-slate-400 mt-1">Perbarui informasi foto dokumentasi ini</p>
                     </div>
                 ...
                 <div class="p-6 sm:p-8 overflow-y-auto custom-scrollbar flex-1">
                     <form action="{{ route('dokumentasi.update', $dokumentasi->id) }}" method="POST" enctype="multipart/form-data" data-turbo-frame="_top" class="space-y-6">
                         @csrf
                         @method('PUT')
                             ...
                             <div class="relative">
                                 <select id="kegiatan_id" name="kegiatan_id" class="form-select w-full h-12 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-transparent focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:focus:border-blue-500/50 rounded-xl px-4 text-[14px] font-medium text-slate-900 dark:text-white transition-all appearance-none cursor-pointer">
                                     ...
                                     @foreach($kegiatan as $k)
                                         <option value="{{ $k->id }}" {{ old('kegiatan_id', $dokumentasi->kegiatan_id) == $k->id ? 'selected' : '' }}>{{ $k->judul }}</option>
                                     @endforeach
                                 </select>
                         ...
                         {{-- Field: File/Foto --}}
                         <div>
                             <label for="file" class="block text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-2 pl-1">Ganti Foto (Opsional)</label>
                             
                             @if($dokumentasi->file)
                             <div class="mb-3 rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 w-full max-w-[200px]">
                                 <img src="{{ asset('storage/'.$dokumentasi->file) }}" class="w-full h-auto object-cover" alt="Current Photo">
                             </div>
                             @endif
                             ...
                             <p class="mt-2 text-[11px] text-slate-500 pl-1">Biarkan kosong jika tidak ingin mengubah foto dokumentasi.</p>
                             @error('file')<p class="mt-2 text-[12px] font-bold text-rose-500 pl-1">{{ $message }}</p>@enderror
                         </div>

                         {{-- Field: Highlight --}}
                         <div class="flex items-center mt-2">
                             <input type="checkbox" id="highlight" name="highlight" value="1" {{ old('highlight', $dokumentasi->highlight) ? 'checked' : '' }}
                         ...
                         <div class="pt-6 flex justify-end gap-3 border-t border-slate-100 dark:border-white/5">
                             ...
                             <button type="submit" 
                                     class="px-5 py-2.5 rounded-xl text-[13px] font-bold text-white bg-amber-600 hover:bg-amber-700 shadow-lg shadow-amber-500/30 transition-all active:scale-95">
                                 Simpan Perubahan
                             </button>
                         </div>
                     </form>
